...
func (ssConn *SshConn) ReceiveWsMsg(wsConn *websocket.Conn, logBuff *bytes.Buffer, exitCh chan bool) {
	...
	for {
		...
		default:
			...
			if err != nil {
				zaplog.Logger.Errorf("reading webSocket message failed:%v", err)
				return
			}
			...
			case wsMsgResize:
				...
				if msgObj.Cols > 0 && msgObj.Rows > 0 {
					if err := ssConn.Session.WindowChange(msgObj.Rows, msgObj.Cols); err != nil {
						zaplog.Logger.Errorf("ssh pty change windows size failed:%v", err)
					}
				}
			case wsMsgCmd:
				...
				decodeBytes := wsData
				if err != nil {
					zaplog.Logger.Errorf("websock cmd string base64 decoding failed:%v", err)
				}
				if _, err := ssConn.StdinPipe.Write(decodeBytes); err != nil {
					zaplog.Logger.Errorf("ws cmd bytes write to ssh.stdin pipe failed:%v", err)
				}
				//write input cmd to log buffer
				if _, err := logBuff.Write(decodeBytes); err != nil {
					zaplog.Logger.Errorf("write received cmd into log buffer failed:%v", err)
				}
			}
		}
	}
	...
	for {
		...
		case <-tick.C:
			...
			if err := flushComboOutput(ssConn.ComboOutput, wsConn); err != nil {
				zaplog.Logger.Errorf("ssh sending combo output to webSocket failed:%v", err)
				return
			}
		case <-exitCh:
			return
...
func (ssConn *SshConn) SessionWait(quitChan chan bool) {
	if err := ssConn.Session.Wait(); err != nil {
		zaplog.Logger.Errorf("ssh session wait failed:%v", err)
		setQuit(quitChan)
	}
}
